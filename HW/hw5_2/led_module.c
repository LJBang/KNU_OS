...
#define GPIO_NUM1	6	// led gpio
#define GPIO_NUM2   21	// button gpio
...
int start_module(void){
	...
	GPIO_IN(GPIO_NUM2);		// set button as input
    GPIO_OUT(GPIO_NUM1);	// set led as output
...
static ssize_t gpio_write(struct file* file, const char* buf, size_t len, loff_t* off){
	if(gpio_get_value(GPIO_NUM2)!=0){ // button is off, off the LED
		GPIO_CLR(GPIO_NUM1);
		return 0;
	}	
	else{
		GPIO_SET(GPIO_NUM1); // button is on, on the LED
		return 0;
	}	
}
